...
class MyDataset(data.Dataset):
    def __init__(self, phase,aug=False):
        ...
        self.list_case_id2 = {'train': r'Data/OpenKBP2/train-pats',
                             'val': r'Data/OpenKBP2/validation-pats',
                             'test': r'Data/OpenKBP2/test-pats'}[phase]
        self.list_datapath = []

        for case_id in os.listdir(self.list_case_id):
            path=os.path.join(self.list_case_id,case_id)
            path2 = os.path.join(self.list_case_id2,case_id)
            ...
            for fn in list_fn:
                ...
                for i in range(slice_num.shape[0]):
                    imagepath = os.path.join(path2, "ct_" + str(slice_num[i]) + ".npy") #0
                    dosepath = os.path.join(path2, "dose_" + str(slice_num[i]) + ".npy") #1
    ...
    def __getitem__(self, index_):
        ...
        npimage = np.concatenate([
                  ...
                  np.zeros((128, 128, 1)),
                  np.zeros((128, 128, 1)),
                  np.zeros((128, 128, 1))], axis=-1).transpose((2, 0, 1))
        
        npdose = np.load(datapath[1])
        npmask = np.zeros((128, 128))
