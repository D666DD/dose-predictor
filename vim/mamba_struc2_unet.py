...
class HybridBlock(nn.Module):
    def __init__(self, img_size, patch_size, embed_dim, depth, channels, num_heads, mlp_ratio=4.0):
        super().__init__()
...
if __name__ == "__main__":
    ...
    structure = torch.randn(8, 5, 128, 128).cuda() 
    t = torch.randint(1000, size=(8,)).cuda()
    model = MambaUnet().cuda()
    y = model(x, structure, t)
    print(y.shape)
