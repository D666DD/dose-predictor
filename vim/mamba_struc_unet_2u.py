...
class MambaUnet(nn.Module):
    def __init__(
        ...
        self.reduce_chan_level3 = nn.Conv2d(
            int(dim * 2**4), int(dim * 2**3), kernel_size=1, bias=bias
        )
        ...
        self.reduce_chan_level0 = nn.Conv2d(
            int(dim * 2**1), int(dim * 2**0), kernel_size=1, bias=bias
        )
        
        self.head = nn.Conv2d(inp_channels, int(dim * 2**0), kernel_size=3, stride=1, padding=1)
        
        self.e_1 = VisionMamba(
            img_size=128, patch_size=2, embed_dim=int(dim * 2**1), depth=4, channels=int(dim * 2**0), **{}
        )
        self.e_2 = VisionMamba(
            img_size=64, patch_size=2, embed_dim=int(dim * 2**2), depth=6, channels=int(dim * 2**1), **{}
        )
        self.e_3 = VisionMamba(
            img_size=32, patch_size=2, embed_dim=int(dim * 2**3), depth=6, channels=int(dim * 2**2), **{}
        )
        self.e_4 = VisionMamba(
            img_size=16, patch_size=2, embed_dim=int(dim * 2**4), depth=8, channels=int(dim * 2**3), **{}
        )



        self.head_struc = nn.Conv2d(inp_struc_channels, int(dim * 2**0), kernel_size=3, stride=1, padding=1)
        ...
        self.d_4 = VisionMamba(
            img_size=16, patch_size=1, embed_dim=int(dim * 2**3), depth=6, channels=int(dim * 2**3), **{}
        )
        self.d_3 = VisionMamba(
            img_size=32, patch_size=1, embed_dim=int(dim * 2**2), depth=6, channels=int(dim * 2**2), **{}
        )
        self.d_2 = VisionMamba(
            img_size=64, patch_size=1, embed_dim=int(dim * 2**1), depth=4, channels=int(dim * 2**1), **{}
        )
        self.d_1 = VisionMamba(
            img_size=128, patch_size=1, embed_dim=int(dim * 2**0), depth=4, channels=int(dim * 2**0), **{}
        )
        self.output = nn.Conv2d(
            int(dim * 2**0), out_channels, kernel_size=3, stride=1, padding=1, bias=bias
        )
        tdim = 24 * 4
        self.time_embedding = TimeEmbedding(tdim)
        self.temb_proj_e1 = nn.Sequential(
            Swish(),
            nn.Linear(tdim, int(dim * 2**1)),
        )
        self.temb_proj_e2 = nn.Sequential(
            Swish(),
            nn.Linear(tdim, int(dim * 2**2)),
        )
        self.temb_proj_e3 = nn.Sequential(
            Swish(),
            nn.Linear(tdim, int(dim * 2**3)),
        )
        self.temb_proj_e4 = nn.Sequential(
            Swish(),
            nn.Linear(tdim, int(dim * 2**4)),
        )
    ...
    def forward(self, inp_img,structure, t):
        ...
        #print("after up", inp_dec_level4.shape)
        #print("from encoder", out_enc_level3.shape)
        inp_dec_level4 = torch.cat([inp_dec_level4, out_enc_level3], 1)
        #print("after cat", inp_dec_level4.shape)
        inp_dec_level4 = self.reduce_chan_level3(inp_dec_level4)
        #print("after reduce", inp_dec_level4.shape)
        out_dec_level4 = self.d_4(inp_dec_level4)
        #print("after decoder", out_dec_level4.shape)
        ...
        inp_dec_level1 = self.reduce_chan_level0(inp_dec_level1)
...
class two_unet(nn.Module):
    def __init__(
        ...
        self.net_model = MambaUnet(
            T=T_,
            ch=ch_,
            ch_mult=ch_mult_,
            attn=attn_,
            num_res_blocks=num_res_blocks_,
            dropout=dropout_,
            dim=dim1,
            inp_channels = inp_channels1,
            inp_struc_channels = inp_struc_channels1,
            out_channels = out_channels1
        )
        self.net_model2 = MambaUnet(
            T=T_,
            ch=ch_,
            ch_mult=ch_mult_,
            attn=attn_,
            ...
            dropout=dropout_,
            dim=dim2,
            inp_channels = inp_channels2,
            inp_struc_channels = inp_struc_channels2,
            out_channels = out_channels2
        )
        self.norm_after_unet1 = nn.BatchNorm2d(out_channels1)
        
    def forward(self, inp_img, structure, t):
        #print("start", inp_img.shape)
        x = self.net_model(inp_img, structure, t)
        #print("first", x.shape)
        prev_x = x
        x = self.norm_after_unet1(x)
        x = torch.cat([inp_img, x], 1)
        #print("second", x.shape)
        x = self.net_model2(x, structure, t)
        #print("third", x.shape)
        return prev_x, x
